	...
	public static class StudentMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (fields.length >= 4) {
				...
				context.write(new Text("Institute: " + institute), new IntWritable(1));
				context.write(new Text("Total Enrolled Students"), new IntWritable(1));
				context.write(new Text("Gender: " + gender), new IntWritable(1));

				if (institute.equalsIgnoreCase("MIT")) {
					context.write(new Text("MIT_" + gender), new IntWritable(1));
				}
			}
		}
	}
